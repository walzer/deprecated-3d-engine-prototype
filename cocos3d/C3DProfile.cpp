...
namespace cocos3d
{
    ...
    void C3DProfile::beginProfile(const std::string& itemname)
    {
        ...
        auto& item = _profileItems[itemname];
        if (item == nullptr)
        {
    ...
    {
#ifdef ENABLE_C3D_PROFILE
        for (auto it = _profileItems.begin(); it != _profileItems.end(); it++)
        {
            auto& item = it->second;
...
C3DProfileDisplay::C3DProfileDisplay():_label(nullptr)
{
}
...
void C3DProfileDisplay::update()
{
    ...
    auto& items = C3DProfile::getInstance()->_profileItems;
    auto it = items.begin();
    char str[128];
    std::string strlabel;
    for (; it != items.end(); it++)
