...
void main()
{
   ...
   vec4 color = basecolor + u_diffuseColor 	 ;
	...
	gl_FragColor = color;
//#ifdef GLOBAL_ALPHA
    gl_FragColor.a *= (1.0 - u_globalAlpha);
//#endif
   ...
   if (u_npointlight > 0)
	...
	vec4 reflectColor = textureCube(cubeMap, v_reflectCoord);
	reflectColor = reflectColor * gl_FragColor.a;
	//ReflectColor = reflectColor * 8;
		//gl_FragColor.rgb  =  reflectColor;
		

		 
	gl_FragColor.rgb  +=  (reflectColor* u_reflectivity).rgb;
	//return;
	

			gl_FragColor.rgb = mix(gl_FragColor.rgb, reflectColor.rgb, u_reflectivity*1.0);
//		else if(u_mixflag == 2)
	//		gl_FragColor.rgb = mix(gl_FragColor, reflectColor.rgb, u_reflectivity*1.0ß);
	//	else if(u_mixflag == 1)
	//		gl_FragColor.rgb = mix(gl_FragColor, gl_FragColor * reflectColor.rgb, u_reflectivity*1.0);
