...
varying vec2 v_texCoord1;                // Texture coordinate (u, v).
...
void main()
{
    basecolor = texture2D(u_diffuseTexture, v_texCoord);
   	lightmap = texture2D(u_diffuseTexture1, v_texCoord1);
	...
	gl_FragColor = mix(color, lightmap, 1.5);//color*0.5 + lightmap * 0.9;
#ifdef GLOBAL_ALPHA
